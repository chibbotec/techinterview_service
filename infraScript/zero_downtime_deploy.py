...
class ServiceManager:
    # 초기화 함수
    def __init__(self, socat_port: int = 9040, sleep_duration: int = 3) -> None:
        self.socat_port: int = socat_port
        self.sleep_duration: int = sleep_duration
        self.services: Dict[str, int] = {
            'techinterview_service_1': 9041,
            'techinterview_service_2': 9042
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'techinterview_service_2', self.services['techinterview_service_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 먼저 이미지 목록 확인
        cmd_list = "docker images | grep '.*/chibbotec/techinterview_service' | grep '<none>'"
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep '.*/chibbotec/techinterview_service' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:9040 -e TZ=Asia/Seoul -v /dockerProjects/chibbotec/techinterview_service/volumes/gen:/gen --pull always ghcr.io/chibbotec/techinterview_service")
    ##
